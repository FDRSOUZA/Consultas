...
let pesquisa_planilha = async () => {
    ...
    await cadastrados.map(item => {//PROCESSOS CADASTRADOS DE EXECUÇÃO FISCAL
        ...
        /*acoes.some(el => {//Pesquisa o nome da classe, tomando por base o array acoes, a ordem que será atendida é do a array acoes
    ...
    if (await array_registro.length > 0) {
        ...
        for (let i = 0; i < await array_registro.length; i++) {
            ...
            if (await (['Execução Fiscal Previdenciária', 'Execução Fiscal (FGTS e Contr. Sociais da LC 110)', 'Execução Fiscal (FNDE)'].includes(array_registro[i].classe))){
                ...
                /*if ( v !== '' && v.length > 3) {
                  let virgula = await array_registro[i].valor[-3];
                //await console.log(n)
                //let virgula = await n.indexOf(",");
                  await console.log(virgula)
                }*/
                /*if (virgula !== -1) {
                    await console.log('____________________________________   '+listconsulta[id_el].numero+' - '+listconsulta[id_el].valor)
                    listconsulta[id_el].valor = await array_registro[i].valor.replace('.', ',');
                }
            }*/
