...
let ler_relatorio = async () => {
    ...
    const fixo = ['Número do processo CNJ', 'Classe SAJ', 'Polo passivo', 'Instância', 'Valor da causa', 'Procurador da última distribuição', 'Distribuído no SAJ', 'Localidade', 'Juízo', 'Inscrições processo principal', 'Somatório inscrições (processo principal e vinculados)', 'Controle de prescrição intercorrente', 'Rating grupo', 'Demanda analytics', 'Processos vinculados']
    var wb = new Excel.Workbook();
    ...
    planilha_alterada = await planilha_lida.map(obj => {//Converte os dados lidos da planilha em um array de objetos definindo as keys
        ...
        let inscricoes = '';
        let somatorio = '';
        if (tipo_execucao.includes(classe) && obj[coluna[9]] !== ' - ') {
            let acumula = 0.00;
            let codigo_somatorio = '';
            if (classe === 'Execução Fiscal (FGTS e Contr. Sociais da LC 110)' || classe === 'Execução Fiscal Previdenciária' || classe === 'Execução Fiscal (SIDA)') { // Se for classe de FGTS, tratar as inscrições para tirar espaço na indicação da situação da inscrição
                let isc_prov =  obj[coluna[9]].replace(/\;/g,"").split("\n");
                let val
                for (let e = 0; e < isc_prov.length; e++) {
                    if (isc_prov[e] !== 'Informação não recuperada') {
                        let prov = isc_prov[e].split(" | ");
                        if (classe === 'Execução Fiscal Previdenciária') {
                            let so_texto = prov[1].split(" - ");
                            prov[1] = so_texto[1];
                        }
                        classe === 'Execução Fiscal (SIDA)' ? val = prov[2].replace(/\./g, '').replace("," , ".") : val = prov[2].substring(2, prov[2].length).replace(/\./g, '').replace("," , ".");
                        acumula = parseFloat(acumula) + parseFloat(val);
                        let novo = `${prov[0]}; ${prov[1].trim()}; ${prov[2]}`;
                        isc_prov[e] = novo;
                    } else {
                        codigo_somatorio = 'Desprezar';
                    }
                }
                inscricoes = isc_prov.join(";\n");
            } else { // Senão somente substituir a barra "|" pelo ponto e vírgula ";"
                inscricoes = obj[coluna[9]].replace(/\|/g,";").split(" ; ").join("; ");
            }
            if (obj[coluna[10]] !== ' - ') {
                if (obj[coluna[10]] !== 'Informação não recuperada') {
                    let prov = obj[coluna[10]].replace(/\./g, "");
                    somatorio = String(prov.substring(2, prov.length)).trim();
                } else {
                    if (codigo_somatorio !== 'Desprezar') {
                        acumula = String(acumula.toFixed(2)); 
                        somatorio = acumula.replace('.',',');
                    }  else {
                        somatorio = obj[coluna[10]].replace('.',',');
                    } 
                }
            }
        } else if (!tipo_execucao.includes(classe) && obj[coluna[4]] !== ' - ') {
            obj[coluna[4]] == '0' ? somatorio = String('0,00') : somatorio = String(obj[coluna[4]]).replace('.',',').trim();
        }
        let parte = '';
...
let ler_input = async () => {
    ...
    await sheets.map(item => {
        ...
        worksheet.eachRow({ includeEmpty: false }, async (row) => {//ler cada uma das linhas da planilha
            let id_relacionado = await tipo_outras_origem.findIndex(element => element === row.values[2]);
            await id_relacionado !== -1 ? acao = tipo_outras_relacionadas[id_relacionado] : acao = row.values[2];
...
let writeexcel = async (nome_arquivo, coluna) => { //funcao para criar o excel de exportacao
    ...
    for(let i = 0; i < listconsulta.length; i++){
        if (coluna == 'Provisória') {
            worksheet.addRow({processo: listconsulta[i].numero, classe: listconsulta[i].classe, procurador: listconsulta[i].prevento, cda_debcad: listconsulta[i].cda, controle_presc: listconsulta[i].prescricao, valor: listconsulta[i].valor, polo: listconsulta[i].parte, juizo: listconsulta[i].juizo, auto: listconsulta[i].registro, rating_grupo: listconsulta[i].grupo, demanda_analytics: listconsulta[i].demanda, vinculados: listconsulta[i].vinculados, finalizado: listconsulta[i].pesquisa}); //loop para escrever o nome dos procuradores e processos na planilha exportada
        } else {
            worksheet.addRow({processo: listconsulta[i].numero, classe: listconsulta[i].classe, procurador: listconsulta[i].prevento, cda_debcad: listconsulta[i].cda, controle_presc: listconsulta[i].prescricao, valor: listconsulta[i].valor, polo: listconsulta[i].parte, juizo: listconsulta[i].juizo, auto: listconsulta[i].registro, rating_grupo: listconsulta[i].grupo, demanda_analytics: listconsulta[i].demanda, vinculados: listconsulta[i].vinculados, finalizado: ''}); //loop para escrever o nome dos procuradores e processos na planilha exportada
        }
    }  
...
let scrape = async () => {
    ...
    if (await listconsulta.length === 0) { 
        ...
        await console.log('Total de ' + (listprocessos.length - listconsulta.length) + ' Processos lidos para consulta' + "\n");
    }
    ...
    if (await array_registro.length > 0) {
        ...
        for (let i = 0; i < await array_registro.length; i++) {
            ...
            } else if (await (!['Execução Fiscal Previdenciária', 'Execução Fiscal (FGTS e Contr. Sociais da LC 110)', 'Execução Fiscal (FNDE)', 'Execução Fiscal (SIDA)', 'Execução Fiscal (informações do débito pendente)'].includes(array_registro[i].classe))){
                if (typeof array_registro[i].valor === 'number') {
                    let conv = String(array_registro[i].valor);
                    let v = await conv.replace('.', ',');
                    console.log(typeof conv + ' ____________________________________');
                    await console.log(v+' - '+array_registro[i].numero+' - '+array_registro[i].classe)
                }
            }*/
            //console.log(' _____________ '+typeof array_registro[i].valor + ' - ' +array_registro[i].numero+' - '+array_registro[i].classe+' - '+array_registro[i].valor);
            if (await dados_autuacao !== '') {
                ...
                //await console.log(`linha ${(id_el+1)}: ${array_registro[i].numero} - ${array_registro[i].classe}`);
            }
            await writeexcel(nome_arquivo_excel, 'Provisória');
        }    
    }
